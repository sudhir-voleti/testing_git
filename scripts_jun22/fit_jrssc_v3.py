...
def build_model(Y_timing, Y_spend, K=3, gamma_m_fixed=0.5):
    N, T = Y_timing.shape
    with pm.Model() as model:
        ...
        gamma_h = 1.0
        # FIX 3: gamma_m FIXED to true value
        gamma_m = gamma_m_fixed
...
def fit_model(Y_timing, Y_spend, K=3, gamma_m_fixed=0.5, draws=500, chains=4, cores=4,
              target_accept=0.9, random_seed=42):
    model = build_model(Y_timing, Y_spend, K=K, gamma_m_fixed=gamma_m_fixed)
    with model:
        idata = pm.sample(
            draws=draws,
            chains=chains,
...
# [rest of extraction functions identical to v2]
...
def compute_ari_forward_filter(dgp, idata):
    ...
    gamma_m_est = float(post['gamma_m'].mean(dim=['chain', 'draw']).values) if 'gamma_m' in post else 0.5
...
def extract_all_metrics(idata, dgp, model, runtime_sec):
    ...
    try:
        ess_gamma_m = az.ess(idata, var_names=["gamma_m"]) if 'gamma_m' in idata.posterior else None
        if ess_gamma_m is not None:
    ...
    if 'gamma_m' in post:
        ...
        metrics['gamma_m_bias'] = 0.0  # Fixed
        metrics['gamma_m_mae'] = 0.0
        metrics['gamma_m_rec'] = 1.0
    if 'alpha_h' in post:
        alpha_h_est = post['alpha_h'].mean(dim=['chain', 'draw']).values
...
def main():
    ...
    parser.add_argument('--gamma_m_fixed', type=float, default=0.5)
    ...
    print("Fitting (gamma_m FIXED=" + str(args.gamma_m_fixed) + "): N=" + str(dgp['N']) + ", T=" + str(dgp['T']) + ", K=" + str(dgp['K']))
    ...
    try:
        idata, model = fit_model(
            ...
            K=dgp['K'],
            gamma_m_fixed=args.gamma_m_fixed,
            draws=args.draws,
            chains=args.chains,
            cores=args.cores,
        ...
        pkl_name = "fit_jrssc_v3_N" + str(dgp['N']) + "_T" + str(dgp['T']) + "_K" + str(dgp['K']) + "_gmFIXED" + str(args.gamma_m_fixed) + "_p0" + str(dgp['pi0']) + "_seed" + str(dgp['seed']) + ".pkl"
