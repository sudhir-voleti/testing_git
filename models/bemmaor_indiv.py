#!/usr/bin/env python3
"""
FILE: model_rfree_27may_v3.py
CREATED: 2026-05-27
CHAT: jmr_27mayv1
STATUS: BEMMAOR-indiv with --T_train flag + forward-filter ARI
PYMC: 5.26.1, ARVIZ: 0.22.0
PURPOSE: Train on T_train periods, OOS on last 20
MATCHES: dgp_rfree_27may_v2.py
"""
import numpy as np
import pymc as pm
...
def main():
    ...
    parser.add_argument('--T_train', type=int, default=None, help='Training periods (default: dgp[T])')
    ...
    T_train = args.T_train if args.T_train is not None else dgp['T']

    print('Fitting BEMMAOR-indiv (r-free) on ' + dgp['world'] + ' world...')
    print('Training on periods 1-' + str(T_train) + ' of ' + str(dgp['T_full']) + ' total')
    t0 = time.time()

    try:
        Y_timing_train = dgp['Y_timing'][:, :T_train]
        Y_spend_train = dgp['Y_spend'][:, :T_train]
        ...
        pkl_name = 'fit_BEMMAOR-indiv-rfree_' + dgp['world'] + '_N' + str(dgp['N']) + '_T' + str(dgp['T']) + '_seed' + str(dgp['seed']) + '.pkl'
        pkl_path = os.path.join(args.output_dir, pkl_name)
        with open(pkl_path, 'wb') as f:
            pickle.dump({
                'idata': idata,
                'dgp_path': args.dgp_path,
                'model_name': 'BEMMAOR-indiv-rfree',
                'world': dgp['world'],
                'seed': dgp['seed'],
                'N': dgp['N'],
                'T': dgp['T'],
