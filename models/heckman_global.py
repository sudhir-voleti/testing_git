#!/usr/bin/env python3
"""
FILE: model_heckman_global_27may.py
CREATED: 2026-05-27
CHAT: jmr_27mayv1
STATUS: Heckman-global with no REs, rho only (population-level statistical coupling)
PYMC: 5.26.1, ARVIZ: 0.22.0
PURPOSE: Zero-heterogeneity ablation for Heckman
MATCHES: dgp_rfree_27may_working.py (all worlds)
NEXT: Test on empty terminals
"""
import numpy as np
import pymc as pm
...
def build_model(Y_timing, Y_spend, K=3):
    ...
    with pm.Model() as model:
        ...
        # Heckman-global: no REs, rho only
        ...
        # Probit for timing (population-level only)
        probit_timing = alpha_h[None, None, :]
        log_p_zero = pt.log(pt.erfc(probit_timing / pt.sqrt(2)) / 2 + 1e-10)
        log_p_pos = pt.log1p(-pt.exp(log_p_zero) + 1e-10)

        # Gamma for spend (population-level only)
        mu_spend = pt.exp(beta_m[None, None, :])
...
def compute_ari_forward_filter(dgp, idata):
    ...
    for i in range(N):
        ...
        for t in range(T):
            ...
            for k in range(K):
                # Probit timing (population-level)
                probit_val = alpha_h_est[k]
...
def main():
    ...
    print('Fitting Heckman-global (r-free) on ' + dgp['world'] + ' world...')
    t0 = time.time()
    ...
    try:
        ...
        pkl_name = 'fit_Heckman-global-rfree_' + dgp['world'] + '_N' + str(dgp['N']) + '_T' + str(dgp['T']) + '_seed' + str(dgp['seed']) + '.pkl'
        pkl_path = os.path.join(args.output_dir, pkl_name)
        with open(pkl_path, 'wb') as f:
            pickle.dump({
                'idata': idata,
                'dgp_path': args.dgp_path,
                'model_name': 'Heckman-global-rfree',
                'world': dgp['world'],
                'seed': dgp['seed'],
                'N': dgp['N'],
                'T': dgp['T'],
