#!/usr/bin/env python3
"""
FILE: model_hurdle_global_27may.py
CREATED: 2026-05-27
CHAT: jmr_27mayv1
STATUS: Hurdle-global with no REs, no coupling
PYMC: 5.26.1, ARVIZ: 0.22.0
PURPOSE: Zero-heterogeneity, zero-coupling baseline
...
def build_model(Y_timing, Y_spend, K=3):
    ...
    with pm.Model() as model:
        ...
        # Hurdle-global: no REs, no coupling whatsoever
        # No gamma_m, no rho, no theta, no u, no v
        ...
        # NBD for timing (population-level only)
        lam = pt.exp(alpha_h[None, None, :])
...
def main():
    ...
    print('Fitting Hurdle-global (r-free) on ' + dgp['world'] + ' world...')
    t0 = time.time()
    ...
    try:
        ...
        pkl_name = 'fit_Hurdle-global-rfree_' + dgp['world'] + '_N' + str(dgp['N']) + '_T' + str(dgp['T']) + '_seed' + str(dgp['seed']) + '.pkl'
        pkl_path = os.path.join(args.output_dir, pkl_name)
        with open(pkl_path, 'wb') as f:
            pickle.dump({
                'idata': idata,
                'dgp_path': args.dgp_path,
                'model_name': 'Hurdle-global-rfree',
                'world': dgp['world'],
                'seed': dgp['seed'],
                'N': dgp['N'],
                'T': dgp['T'],
